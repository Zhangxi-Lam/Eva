...
def main():
    ...
    try:
        ...
        # Get port
        arm = input("Which arm (l/r): ").strip().lower()
        port = constants.LEFT_ARM_PORT if arm == "l" else constants.RIGHT_ARM_PORT
        print(f"Connecting to port: {port}")
        
        # Configure robot
        robot_config = SO100FollowerConfig(port=port, id=constants.RIGHT_ARM_ID, calibration_dir=Path(constants.CALIBRATION_DIR))
