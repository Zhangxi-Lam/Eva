...
class EvaRobot:
    ...
    def __init__(self, config: EvaRobotConfig):
        ...
        self.bus = FeetechMotorsBus(
            ...
            calibration=self._load_calibration(config.calibration_dir)
        )
    ...
    def reach_position(self, target: RobotPosition, current: RobotPosition)->bool:
        if not target:
            return False
    ...
    def move_to_position(self, target: RobotPosition):
        if not self.is_connected:
            raise DeviceNotConnectedError(f"{self.config.id} is not connected.")
    ...
    def move_to_position_in_loop(self, target: RobotPosition):
        ...
        while True:
            ...
            if self.reach_position(last_position, current):
                current = self.get_current_position()
                logger.warning(f"Moving to position {target} stuck at {current}!")
                return
